...
import AccessoriesList from '../API/AccessoriesApi'
        ...
        <div>
            <section className="text-gray-600 body-font">
                <div className="container px-2 sm:px-3 md:px-1 lg:px-5 pt-10 pb-24 mx-auto">
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:gap-3 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 items-center justify-center">

                        {AccessoriesList.AccessoriesData.map((item) => (
                            ...
                            <div key={item.id} className="lg:w-[235px]  p-4  sm:w-full border-2 border-black m-2">
                                ...
                                <div className="mt-4">
                                    ...
                                    <h2 title={item.name} className="text-gray-900 text-xs sm:text-sm md:text-base lg:text-md font-medium">{shortenTitle(item.name, 15)}</h2>

                                    <p className="mt-1 text-black font-medium mb-2"><span className='line-through mr-2'>{item.old_price}</span><br /><span className='text-red-500 text-sm'>{item.price_off}</span> <br />₹{item.price}</p>
                                    {/* Buy Now button */}
